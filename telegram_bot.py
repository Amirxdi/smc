...
async def auto_level_scan_loop(app: Application) -> None:
    ...
    while True:
        try:
            ...
            logger.exception("Level scan loop error: %s", e)
...
async def auto_trendline_scan_loop(app: Application) -> None:
    ...
    while True:
        try:
            ...
            if breakouts:
                await send_message_safe(app, chat_id, format_trendline_results(breakouts))

            await asyncio.sleep(TRENDLINE_INTERVAL_MIN * 60)
        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.exception("Trendline scan loop error: %s", e)
            await asyncio.sleep(30)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        ...
        f"Trendline: every <b>{TRENDLINE_INTERVAL_MIN}min</b>\n"
        f"Sessions: <b>{session_info}</b>\n"
        f"Min score: <b>{MIN_SIGNAL_SCORE}</b>\n\n"
        f"Commands:\n"
        f"/scan — Manual level scan\n"
        f"/trendline — Manual trendline scan\n"
        f"/status — Config check\n"
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        result = await run_level_scan()
        await update.message.reply_html(format_level_scan_summary(result))
    except Exception as e:
    ...
    try:
        ...
        for ex in exchanges:
            ...
            try:
                ...
                await m.close()
        await update.message.reply_html(format_trendline_results(breakouts))
    except Exception as e:
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not errors:
        ...
        lines.append(f"📊 Trendline: <b>{TRENDLINE_INTERVAL_MIN}min</b>")
        labels = [EXCHANGE_CONFIGS[e]["label"] for e in get_enabled_exchanges()]
